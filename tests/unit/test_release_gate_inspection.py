...
from pmpe.support_package import PackageContractError, SupportPackageContract, _load_release_candidate
...
def _packet(root: Path, mutation: str = "", *, gated: bool = True) -> tuple[str, dict[str, Any]]:
    ...
    ledger.append(
        event_type="contract_validated", state="VALIDATED", subject_digest=subject,
        blob_digests=(contract_blob, plan_blob, receipt_blob),
        payload={"contract_digest": contract_blob, "plan_digest": plan["plan_digest"],
                 "approval": {"status": "VERIFIED", "authority": "fixture-human",
                              "receipt_digest": receipt["receipt_digest"],
                              "receipt_blob_digest": receipt_blob}},
    )
    app = ledger.put_blob(b"def health():\n    return {'status':'ok'}\n")
    binding = ledger.put_blob((subject + "\n").encode())
    candidate = ledger.put_blob(canonical_json_bytes(
        {"app.py": app, "package-contract-digest.txt": binding}
    ))
    payload: dict[str, Any] = {"candidate_digest": candidate}
    terminal_blobs = [candidate, app, binding]
    if gated:
        evidence = {
            "attempt": 1, "contract_digest": subject, "plan_digest": plan["plan_digest"],
            "candidate_digest": candidate,
            "gates": [{"gate_id": "GATE-001", "acceptance_criterion_refs": ["AC-001"],
                       "status": "PASS", "criterion_results": [
        ...
        ledger.append(
            event_type="release_gates_evaluated", state="VERIFYING", subject_digest=subject,
            blob_digests=(gate_blob, candidate, app, binding), payload=evidence,
        )
    ...
    terminal = ledger.append(
        event_type="release_ready", state="RELEASE_READY", subject_digest=subject,
        blob_digests=terminal_blobs, payload=payload,
    )
...
@pytest.mark.parametrize("mutation", [
    "candidate_digest", "contract_digest", "plan_digest", "blob-only", "missing-pointer",
    "gate-omitted", "failed-criterion", "plan-stripped",
])
...
def test_support_candidate_rejects_m3_even_with_matching_expected_head(tmp_path: Path) -> None:
    ...
    with pytest.raises(PackageContractError, match="gate"):
        _load_release_candidate(
            tmp_path, "inspection", SupportPackageContract(contract, canonical_digest(contract)),
...
def test_inspection_can_check_existing_external_head_anchor(
    ...
    args = ["barebones", "inspect", "inspection", "--repository-root", str(tmp_path),

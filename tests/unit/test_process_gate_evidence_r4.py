...
@pytest.mark.parametrize(
    "marker",
    [None, {"invalid_json": True}, {"exit_code": "timeout"}, "wrong_candidate", "wrong_plan"],
)
def test_negative_control_rederives_observer_markers(marker: Any) -> None:
    ...
    context = {"candidate_digest": candidate, "plan_digest": "sha256:" + "a" * 64}
    if marker in ("wrong_candidate", "wrong_plan"):
        context["candidate_digest" if marker == "wrong_candidate" else "plan_digest"] = (
            "sha256:" + "b" * 64
        )
        with pytest.raises(ValueError, match="MUTANT_RELEASE_CONTEXT_MISMATCH"):
            validate_process_gate_evidence(
                binding,
                evidence,
                criterion_ids=["AC-1"],
...
def test_digest_evidence_rejects_foreign_approval_context(changed: bool) -> None:
    import hashlib
    import json
    ...
    draft = {"test": "draft"}
    plan = {"contract_digest": contract_digest}
    ...
    if changed:
        freeze["artifacts"]["contract"] = put({"foreign": "contract"})
        evidence["approval_freeze_digest"] = put(freeze)
        with pytest.raises(ValueError, match="APPROVAL_CONTRACT_CONTEXT_MISMATCH"):
            validate_process_gate_evidence(
                binding,
                evidence,
                criterion_ids=["AC-1"],

...
12 of 18 PRs drew a REQUEST_CHANGES round; every finding was fixed and
...
- The 20 completion conditions of the V2 specification are confirmed on this
  tree; conditions 19–20 (draft PR only, no auto-merge) hold for the *product*
  (the run engine's draft-PR handoff never merges, PD-08) — the series' own
  merges to `main` were explicit owner-authorized delivery operations.

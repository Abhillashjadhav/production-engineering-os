...
| Reproducibility versus vulnerability intelligence freshness | A pinned scanner/advisory snapshot is reproducible but can miss newly disclosed critical vulnerabilities. | Pin tools and evidence digests while separately enforcing an approved advisory maximum age; stale, unverifiable, or unavailable intelligence blocks only from a pre-rollout/proven-safe state, cleans staging, or rolls back active canary/changed-production exposure before blocking. |

...
| 2 | TBD | Specification ingestion | `pmpe validate` loads JSON/YAML MVP specs, enforces the schema, normalizes to typed MvpSpec, rejects malformed input with exit 2 | TRD ingestion requirements; PRD spec-input contract | 18 unit tests (schema + normalizer + packaged-schema sync guard) passed; CLI smoke on golden + malformed fixtures | pending | pending | Structural validation only — semantic requirement validation lands in PR 3 |

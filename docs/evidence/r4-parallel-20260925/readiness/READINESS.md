...
| GATE-004 during retained inspection | The validation path for a claimed PASS calls `validate_process_gate_evidence`, which unconditionally raises `FRESH_GENERATION_NOT_MECHANICALLY_VERIFIED` for `generation_provenance`. This does not mean every halted or NOT_EVALUATED retained run is rejected. | The retained reader must verify the same supported PASS evidence as the runtime. A runtime-only change cannot close this gate. |
...
1. Advance the independent architecture and freshness workstreams in parallel after their respective decisions. Within architecture, complete the source/bootstrap design, implementation and permitted review in order. Within freshness, resolve its design decision, establish a usable provenance capability, then implement and review the runtime, retained-reader and invocation integration. If freshness capability remains unestablished, stop that delivery unit without minting an approval or running the model; safe architecture/source/replay work can continue.
2. Integrate the completed workstreams and finish permitted final-source reviews; preserve exact source/publication identities. Final draft regeneration waits for both required implementations. The prior security-screening-stopped active external-cache and retained-context-binding adversarial rechecks remain UNVERIFIED. Static review and ordinary replay do not replace them.

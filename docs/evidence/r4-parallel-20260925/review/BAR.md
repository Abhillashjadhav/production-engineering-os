...
4. **Does a check fail before and pass after? No — stop until established.** First add and run the data-only review check, recording the missing follow-up acceptance matrix as RED before writing that matrix; then verify the matrix against actual retained files and record GREEN.

...
This PR's head also carries #227 `f6df6a2`, `feb49dc` and `9692a9e`, which add checker rounds 19 (pinned candidate trees and the measure's value domain), 20 (the exact recorded runtime string) and 21 (exact JSONL serialization). They change only `docs/evidence/r3-repair-20260924/check_replay_complete.py`, its tests, its RED/GREEN logs and `BAR.md`, and no file under `src/`, `scripts/` or `tests/`, so the replay and scanner evidence from `80cd230` stands. The checker run in `replay-checker.txt` was refreshed on this head.

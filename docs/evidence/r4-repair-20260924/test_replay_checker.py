...
class ReplayCheckerRegression(unittest.TestCase):
    def run_check(self, root, optimized=False):
        return subprocess.run(
            [sys.executable, '-O' if optimized else '-B', str(CHECKER), str(root),
             '--packet', str(PACKET), '--peos-source', str(SOURCE), '--case', root.name],
    ...
    def test_optimized_interpreter_rejects_missing_boundary(self):
        self.mutate('retained', lambda root: self.change_rows(
            root, 'digest-checks.jsonl', lambda rows: rows.pop()), True)
    ...
    def test_fake_matching_inventories_rejected(self):
        def fake(rows):
            for row in rows:
                row.update(expected_inventory_digest='sha256:' + '0' * 64,

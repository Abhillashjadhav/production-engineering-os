...
| `PRODUCTION_APPROVAL_REQUIRED → STAGING_FAILED` | Canary teardown completed with deterministic zero canary/production exposure, but staging is missing, stale, unavailable, or fails revalidation; before any staging cleanup mutation, CP has durably recorded a distinct cleanup idempotency key and DeploymentAttempt | Approval-decision/outage stop event, complete canary-teardown result, zero-exposure proof, failed/missing staging revalidation and freshness policy, pre-mutation staging-cleanup attempt/step plan, complete cleanup result, and zero remaining staging resources | CP invalidates rollout/approval evidence, persists the staging-cleanup attempt first, tears staging down idempotently, and records staging-failed only after cleanup succeeds | Failed/indeterminate staging cleanup → `BLOCKED` plus incident with active-resource status; expired exposure authorization or failed canary teardown uses `CANARY_FAILED`/rollback instead. | No; both teardowns are pre-authorized |

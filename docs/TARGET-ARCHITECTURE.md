...
| `STAGING_DEPLOYED → CANARY_DEPLOYED` | Named, unexpired, unrevoked canary authorization and policy plus an immediate staging revalidation PASS within the approved freshness window, a pre-mutation idempotency key/DeploymentAttempt, a monotonic authorization lease/fence that the rollout adapter can validate at every mutating step and final admission, and a fully successful canary mutation | Current staging EvidenceBundle and environment/integration/migration/security snapshot, freshness policy and evaluation time, bounded cohort/traffic/window, rollback readiness, authorization identity/time/reason/subjects/expiry/version, authorization-fence observations immediately before every traffic/instance/config/data mutation and after the complete adapter result, append-only DeploymentAttempt/DeploymentResult, and a final still-valid authorization observation bound to that result | CP revalidates mutable staging state and authorization, persists the attempt, and supplies the exact fence to an adapter that refuses each mutation after expiry/revocation/version change. An independent watchdog observes authorization through the operation. CP admits canary only after the successful complete result and a final matching authorization observation | Failed/stale revalidation → `STAGING_FAILED`; cancellation, invalid authorization, or adapter/infrastructure failure before any canary mutation uses the healthy staging-teardown `BLOCKED` path. Authorization expiry/revocation/fence change during or after any completed or unknown mutation, including after an otherwise successful adapter response but before admission, uses the direct pre-journaled rollback transition below; stale authority can never admit canary. | Per approved canary policy |

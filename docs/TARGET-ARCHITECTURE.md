...
| `PRODUCTION_APPROVAL_REQUIRED → STAGING_DEPLOYED` | The production-approval request is rejected, revoked, or expires without a safety breach, **or** a non-decision approval/identity/permission service outage makes continued waiting unsafe or impossible; the separate canary exposure authorization remains valid through cleanup; CP has durably recorded an exact-subject canary-teardown idempotency key and DeploymentAttempt before any traffic/instance/config/data mutation; teardown completes; and deterministic zero canary/production exposure is proven | Pre-mutation teardown attempt/step plan, successful complete DeploymentResult, either the decision event or an outage/stop event proving no decision was inferred, still-valid exposure authorization through teardown, zero-exposure proof, current staging deployment/revalidation evidence, and invalidated canary/production evidence | CP persists the teardown attempt first; the adapter removes canary exposure idempotently; CP admits staging only after the complete result and zero-exposure proof. An outage is never reinterpreted as rejection or approval; after dependency recovery a new canary window and new approval are required | Expired exposure authorization, guardrail breach, or indeterminate/failed teardown uses `CANARY_FAILED` and rollback; failed/stale staging uses `STAGING_FAILED`; if the dependency remains unavailable, the staging-deployed safe-cancellation path may pre-journal staging teardown and enter `BLOCKED`. No after-the-fact teardown inference or stale approval survives. | No; teardown is pre-authorized by rollout policy |

...
| `BLOCKED → <recorded resume state>` | External dependency/permission resolved with every upstream contract/repository/base/candidate digest unchanged and no unresolved staging, canary, or changed-production resource/exposure owned by this rollout | Resolution identity, changed external-state evidence, prior state/action, rollout-scoped zero-exposure or completed-rollback proof where relevant, restored last-known-good production subject where applicable, and unchanged upstream digests | CP rechecks then resumes the exact interrupted gate | Recheck failure remains blocked; changed inputs must use an invalidation transition below; no skip to a later or terminal state. Existing last-known-good production may remain active and monitored. | If permission/decision |

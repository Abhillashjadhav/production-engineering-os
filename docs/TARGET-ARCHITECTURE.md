...
| `PRODUCTION_APPROVAL_REQUIRED → PRODUCTION_DEPLOYED` | Named, unexpired, unrevoked approval bound to merged source, artifact/config/migration/target/policy; active canary and target/integration/security/SLO revalidation PASS within the approved freshness window; a durably persisted exact-subject idempotency key/DeploymentAttempt before any production mutation; and an approval lease/fence that the protected-environment adapter can validate at every mutating step and final admission | GitHub merge/head/SHA; approval identity/time/reason/digests/expiry and monotonic authorization version; protected-environment authorization; approval-status/fence observations immediately before every traffic/instance/config/data mutation and after the complete adapter result; active canary/target snapshot; freshness policy/evaluation time; pre-mutation attempt/target/step plan; completed steps; successful complete DeploymentResult; and final still-valid approval observation bound to that result | CP revalidates the canary, target, and approval first, persists the attempt second, and supplies the exact approval fence to an adapter that refuses each mutation after expiry/revocation/version change. An independent watchdog observes authorization through the operation. CP admits production only after the successful result and a final matching approval observation | Inactive/stale canary before mutation revokes the pending approval and uses staging reset; active canary breach/missing signal uses `CANARY_FAILED`; stale/mismatched approval before mutation is refused. Approval expiry/revocation/fence change during or after any mutation, including after an otherwise successful adapter response but before state admission, is an interrupted unauthorized promotion and uses the direct pre-journaled rollback path; no stale approval can admit production. | Yes |

...
| `PRODUCTION_APPROVAL_REQUIRED → PRODUCTION_DEPLOYED` | Named approval bound to merged source, artifact/config/migration/target/policy and a durably persisted exact-subject idempotency key/DeploymentAttempt before any production mutation | GitHub merge/head/SHA, approval identity/time/reason/digests, protected-environment authorization, pre-mutation attempt/target/step plan, completed steps, and successful complete DeploymentResult | CP persists the attempt first; production adapter idempotently promotes only the merge-bound immutable artifact and appends each step | Unmerged, stale, mismatched, or expired subject is refused before mutation; after any completed or unknown mutation, adapter failure/interruption or a missing result must use the rollback transition below. | Yes |

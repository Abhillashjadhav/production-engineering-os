...
Entry to the table has a pre-parse invariant. Before inspecting submitted bytes, the
intake adapter uses a caller retry key to append an immutable `IntakeReceipt` containing
a control-plane-assigned opaque lineage ID, a distinct ingestion-attempt ID, receipt
time, raw/envelope digest and immutable object reference, publisher/channel metadata,
and any correction reference. Only after that append is durable may the adapter
acknowledge receipt and enter `CONTRACT_RECEIVED`. Persistence failure refuses the
request without parsing; retrying the same key returns the same receipt. A valid
...
|---|---|---|---|---|---|
| `CONTRACT_RECEIVED → CONTRACT_INVALID` | Durable IntakeReceipt and received bytes/manifest | Immutable lineage and ingestion-attempt IDs, receipt/raw-object reference, parse/schema/compiler diagnostics, and source digest where parseable | CP validates only after matching bytes to the receipt | Malformed, unsupported, lossy, ownership-invalid. No downstream work starts; retain the rejected attempt and return diagnostics. | No |
| `CONTRACT_INVALID → CONTRACT_RECEIVED` | Corrected bytes plus the server-issued lineage or original-attempt reference | Original lineage/attempt, a new durable correction IntakeReceipt and attempt ID under that lineage, correction relation, new source/version/digest where parseable, and PM approval when required | PM submits; intake persists before parsing; CP never edits the rejected attempt | Missing/invalid correction reference creates a new lineage and possible-duplicate finding; same version with changed digest is refused; old record retained. | PM approval where required |
...
| `PRODUCT_INPUT_REQUIRED → CONTRACT_RECEIVED` | Superseding bundle/PCR decision, server-issued lineage/original-attempt reference, and, when downstream work exists, its issue/branch/draft-PR/worktree disposition | Original lineage/attempt, new durable correction receipt/attempt and relation, resolution, approver, new version/digest, atomic-scope comparison, and durable disposition record | PM submits; intake persists before parsing; CP halts worktrees and either reuses the same issue/branch/draft PR when the atomic outcome is unchanged or marks them superseded and requires a new issue/primary PR when scope changed | Invalid correction reference starts a new lineage with a duplicate finding; unresolved/unauthorized input or any undispositioned active candidate remains product-input-required; old contract/candidate evidence is retained and cannot execute. | Yes |
| `CONTRACT_RECEIVED → CONTRACT_APPROVED` | Durable IntakeReceipt and valid complete approved bundle | Lineage/attempt IDs, 100% contract gates, rule-set digest, approval metadata | CP admits and locks | Any blocker routes to invalid/input-required; no rollback needed. | Existing PM approval required |

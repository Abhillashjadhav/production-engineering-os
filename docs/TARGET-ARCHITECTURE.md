...
| `PRODUCTION_APPROVAL_REQUIRED → STAGING_DEPLOYED` | The production-approval request is rejected, revoked, or expires without a safety breach, **or** a non-decision approval/identity/permission service outage makes continued waiting unsafe or impossible; the separate canary exposure authorization remains valid through cleanup; CP has durably recorded an exact-subject canary-teardown idempotency key and DeploymentAttempt before any traffic/instance/config/data mutation; teardown completes; deterministic zero canary/production exposure is proven; and staging revalidation is current and passes | Pre-mutation teardown attempt/step plan, successful complete DeploymentResult, either the decision event or an outage/stop event proving no decision was inferred, still-valid exposure authorization through teardown, zero-exposure proof, current passing staging deployment/revalidation evidence, and invalidated canary/production evidence | CP persists the teardown attempt first; the adapter removes canary exposure idempotently; CP admits staging only after the complete result, zero-exposure proof, and current staging PASS. An outage is never reinterpreted as rejection or approval; after dependency recovery a new canary window and new approval are required | Expired exposure authorization, guardrail breach, or indeterminate/failed canary teardown uses `CANARY_FAILED` and rollback; missing/stale/failed staging uses the direct cleanup transition below; if the dependency remains unavailable after a staging PASS, the staging-deployed safe-cancellation path may tear staging down and enter `BLOCKED`. | No; teardown is pre-authorized by rollout policy |

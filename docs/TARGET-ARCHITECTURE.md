...
Entry to the table has a pre-parse invariant. The intake adapter accepts bytes only
into a bounded, encrypted, access-isolated, deletable quarantine whose contents never
enter logs or the immutable evidence store. While receiving, it computes the envelope
digest and uses a caller retry key to append a safe-metadata-only immutable
`IntakeReceipt`: control-plane-assigned opaque lineage ID, distinct ingestion-attempt
ID, receipt time, digest, quarantine-handle token, publisher/channel metadata, and any
correction reference. The handle is opaque, expires, and is never sufficient to fetch
bytes without intake authorization. Only after that receipt is durable may the adapter
acknowledge receipt and enter `CONTRACT_RECEIVED`. It then runs size, content-type,
malware, secret, privacy/data-classification, parse, and schema admission checks before
copying an admitted, policy-allowed payload to content-addressed immutable storage.
...
attestation. Persistence failure deletes the quarantine object and refuses the request;
retrying the same key returns the same receipt or its terminal disposition. A valid
correction reference creates a new attempt under the original lineage; a missing or
invalid reference creates a new lineage plus a possible-duplicate finding. Claimed
contract/slice identifiers never determine lineage.

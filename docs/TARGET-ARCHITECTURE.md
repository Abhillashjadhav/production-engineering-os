...
| `BLOCKED → REPOSITORY_ANALYSED` | Contract unchanged but repository/base/toolchain/governance input changed, no unresolved environment exposure, and a fresh exact-SHA RepositorySnapshot is available | Old/new input digests, changed-input classification, invalidated downstream evidence, active-work disposition, and fresh snapshot/scanner versions | CP re-admits repository facts, then requires architecture and every downstream artifact to be reproposed/revalidated | Contract/product change uses contract intake; missing safe-state proof or snapshot remains blocked. | Owner for change classification |
...
| `PRODUCTION_APPROVAL_REQUIRED → STAGING_DEPLOYED` | Canary was safely removed, or its approval-bound evidence expired without a safety breach, with deterministic zero canary/production exposure | Revoked/expired production-approval request, zero-exposure proof, current staging deployment/revalidation evidence, and invalidated canary/production evidence | CP resets to staging; it performs no production mutation and requires a new current canary window and new production approval | Active/indeterminate canary uses `CANARY_FAILED`; failed/stale staging uses `STAGING_FAILED`; no stale approval survives the reset. | No |

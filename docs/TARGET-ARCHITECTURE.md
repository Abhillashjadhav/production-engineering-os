...
| `STAGING_FAILED → PR_MERGED` | Failure is deterministically classified as freshness-only expiry/staleness with no candidate, contract, architecture, policy, or infrastructure defect; cleanup proved zero resources; and the exact merged artifact remains admissible | Freshness rule/result, independent no-defect classification, complete/no-resource cleanup result, exact unchanged merge/artifact/config subjects, invalidated staging evidence, and new deployment-attempt eligibility | CP re-admits only the unchanged merged subject and requires a fresh staging deployment; it does not claim the removed environment still exists | Any candidate/product/policy drift uses repository analysis or product input; unavailable infrastructure uses `BLOCKED`; missing cleanup or classification proof stays failed. | No |
...
enter `BUDGET_EXCEEDED`. Both stopped states record the exact safe post-cleanup/rollback
